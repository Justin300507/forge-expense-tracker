...
const Header = ({ dark, setDark }) => {
  ...
  return (
    <header className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
      <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Hello, {displayName}!</h2>
      <div className="flex items-center gap-3">
        ...
        <button onClick={handleLogout} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors flex items-center gap-1">
          <LogOut size={18} />
        </button>
      </div>
    </header>
